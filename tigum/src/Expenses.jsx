...
function Expenses() {
  ...
  async function updateExpense(e) {
    ...
    axios.put(`http://localhost:5000/updateexpense/${updateId}`, data)
    .then((response) => {
      console.log(response.data);
  ...
  async function deleteExpense(e) {
    if(diffDays > 7) {
        ...
        axios.delete(`http://localhost:5000/deleteexpense/${deleteId}`)
        .then((response) => {
        console.log(response.data);
        window.location.reload();
        })
            ...
            return (
              <div key={index} className="border-bottom border-bottom-dark">
                ...
                <div className="d-flex flex-row justify-content-between">
                  <div>
                    <p>Date: {LocalDate}</p>
                    <p>From Budget: {list.budget_name} {list.is_deleted ? "(Budget Deleted)" : ""}</p>
                  </div>
                </div>
                <br />
                {modalOpen && (
                    <Modal show={true} backdrop={false} centered>
                        ...
                        <button onClick={() => {setopenExpenseUpdateForm(true) 
                        ...
                        <button onClick={() => setopenExpenseDeleteForm(true)}>Delete</button>
                ...
                {openExpenseUpdateForm && (
                    <Modal show={true} backdrop={false} centered>
                        ...
                        <form onSubmit={updateExpense}> 
                            <label>Name:</label>
                            <input type="text" onChange={(e) => setExpenseName(e.target.value)} placeholder={readObject.expense_name}/>
                            <br />
                            <label>Amount:</label>
                            <input type="number" onChange={(e) => setExpenseAmount(e.target.value)} placeholder={readObject.expense_amount}/>
                            <br />
                            <label>Category:</label>
                            <input type="text" value={expenseCategory}/>
                            <select onChange={(e) => setExpenseCategory(e.target.value)} placeholder={readObject.expense_category}>
                            ...
                            <button onClick={() => setopenExpenseUpdateForm(false)}>CLOSE</button>
                        </Modal.Body>
                    </Modal>
                )}
                {openExpenseDeleteForm && (
                    <Modal show={true} backdrop={false} centered>
                        ...
                        <button onClick={() => setopenExpenseDeleteForm(false)}>NO</button>
                        </Modal.Body>
                    </Modal>
                )}
              </div>
